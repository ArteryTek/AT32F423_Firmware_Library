...
/** @addtogroup 423_ADC_conversion_abort ADC_conversion_abort
  * @{
  */

__IO uint16_t adc1_ordinary_valuetab[3] = {0};
__IO uint32_t dma_trans_complete_flag = 0;
...
static void adc_config(void)
{
  ...
  adc_ordinary_channel_set(ADC1, ADC_CHANNEL_4, 1, ADC_SAMPLETIME_92_5);
  adc_ordinary_channel_set(ADC1, ADC_CHANNEL_5, 2, ADC_SAMPLETIME_92_5);
  adc_ordinary_channel_set(ADC1, ADC_CHANNEL_6, 3, ADC_SAMPLETIME_92_5);
...
int main(void)
{
  ...
  while(1)
  {
    ...
    if((conversion_times_index == dma_trans_complete_flag) || (error_times_index != adc1_overflow_flag))
    {
      /* printf flag when error occur */
      error_times_index = adc1_overflow_flag;
      at32_led_on(LED3);
      at32_led_on(LED4);
      printf("conversion stop or error occur\r\n");
    ...
    else
    {
      ...
      conversion_times_index = dma_trans_complete_flag;
