...
/** @addtogroup 423_TMR_hang_mode TMR_hang_mode
  * @{
  */
...
int main(void)
{
  ...
  /* hang mode test -----
    ...
    * the timxclk is fixed to apb2_freq, the three timers are running at:
      tmrx frequency = tmrx clock counter/ (timx_period + 1) = 500 khz.
  */
  /* tmr2 and tmr3 counter mode configuration */
  timerperiod = ((crm_clocks_freq_struct.apb2_freq) / 500000 ) - 1;
