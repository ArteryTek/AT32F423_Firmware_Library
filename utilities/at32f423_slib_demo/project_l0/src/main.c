...
#define SLIB_START_ADDR                  0x0803E000
#define SLIB_INST_ADDR                   0x0803F000
#define SLIB_END_ADDR                    0x0803F800
...
#if defined (USE_SLIB_FUNCTION)
uint32_t dw_start_sector, dw_inst_start_sector, dw_end_sector;
#endif
...
flash_status_type slib_enable(void)
{
  ...
  dw_inst_start_sector = SECTOR_NUM(SLIB_INST_ADDR);         /* slib data sector */
  dw_end_sector = SECTOR_NUM(SLIB_END_ADDR);                /* slib end sector */
  status = flash_slib_enable(TEST_SLIB_PSW, dw_start_sector, dw_inst_start_sector, dw_end_sector);
  return status;
}
#endif
